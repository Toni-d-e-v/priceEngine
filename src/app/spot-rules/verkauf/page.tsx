...
export default function SpotRulesVerkaufPage() {
  ...
  const verkaufRules = allRules.filter(r => r.direction === 'verkauf' && r.metalId === selectedMetalId);
  const currentRate = rates.find(r => r.metalId === selectedMetalId);
  const spotPrice = currentRate?.rateEurPerGram ?? 0;

  const activeRule = verkaufRules.find(r => r.isActive && r.ruleType === 'standard');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-muted-foreground">
        {metalNames[selectedMetalId]} Verkauf Spotpreis Aktuell:{' '}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[200px_1fr]">
        ...
        <MetalSelector
          ...
          rules={allRules.filter(r => r.direction === 'verkauf')}
        />
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Standardregeln Verkauf</CardTitle>
            </CardHeader>
            <CardContent>
              <StandardRulesTable rules={verkaufRules} onEdit={handleEdit} onDelete={handleDelete} />
            </CardContent>
          </Card>

          <Card>
            <CardHeader>
              <CardTitle className="text-base">Sonderregeln Verkauf</CardTitle>
            </CardHeader>
            <CardContent>
              <SpecialRulesTable rules={verkaufRules} onEdit={handleEdit} onDelete={handleDelete} />
            </CardContent>
          </Card>
        </div>
      </div>
